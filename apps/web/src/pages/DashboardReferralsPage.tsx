...
import { Button, Card, CardContent, ProtectedRoute, Skeleton } from "@luxero/ui";
...
function ReferralsContent() {
  ...
  useEffect(() => {
    api
      .get<ApiResponse<ReferralsData>>("/api/me/referrals")
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div>
          ...
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight">
            Refer Friends, <span className="text-gold-gradient">Earn Tickets</span>
          </h1>
                ...
                <>
                  <a href={shareLinks.whatsapp} target="_blank" rel="noopener noreferrer">
                    <Button
                      variant="outline"
                      size="icon"
                      className="border-gold/30 hover:bg-gold/10 h-10 w-10"
                    >
                      <svg
                        className="w-4 h-4"
                        viewBox="0 0 24 24"
        ...
        <div className="rounded-2xl border border-gold/30 bg-gradient-to-br from-gold/10 via-card to-gold/5 p-6">
                ...
                <div key={tier.threshold} className="flex-1 text-center">
                  <div
                    className={`h-3 rounded-full mb-2 ${
                      isAchieved ? "bg-gold" : isCurrent ? "bg-gold/50" : "bg-border"
                    }`}
                  />
          ...
          {data.referralsToNextTier > 0 ? (
            <p className="text-xs text-muted-foreground text-center">
              <span className="font-medium text-gold">{data.referralsToNextTier}</span> more active
              referrals to unlock{" "}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              },
              {
                ...
                description:
                  "When your referred friend buys tickets within 30 days, they count as active",
              },
              {
                step: "03",
                icon: Ticket,
                ...
                description:
                  "Earn 2-10 tickets per competition based on your tier (5, 10, or 15 active referrals)",
              },
            ].map((item) => (
